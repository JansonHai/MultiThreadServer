...
{
	fl_debug_log("[Gate_Watchdog]: Stoping Gate watchdog Server");
	close(s_watchdog_listen_fd);
	s_run_state = 0;
	for (int i=0;i<32;++i)
	{
		...
		if (FD_ISSET(s_watchdog_listen_fd, &readset))
		{
			...
			else
			{
				for (i=0;i<32;++i)
				{
					...
					fl_debug_log("[Gate_Watchdog]: Accept client from %s:%d,clientfd = %d",inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port),clientfd);
					break;
				}
			}
			--selectn;
...
static void s_watchdog_handle_message(struct fl_message_data * message)
{
	fl_debug_log("[Gate_Watchdog]: client %d message handle,length = %d", message->fd, message->length);
	...
	fl_debug_log("[Gate_Watchdog]: client %d message: %s", clientfd, tmp.c_str());
	while (*ch != '\0')
	{
		if (*ch == ',')
		{
