...
bool fl_start_login_watchdog_server()
{
	...
	{
		fl_log(2, "[Login_Watchdog]: login watchdog server socket failed, errno:%d\n", errno);
		return false;
	}
	...
	if (-1 == (bind(s_watchdog_listen_fd,(struct sockaddr*)&server_addr,sizeof(struct sockaddr))))
	{
		...
		fl_log(2, "[Login_Watchdog]: login watchdog server bind error, errno:%d\n", errno);
		return false;
	}
	...
	{
		close(s_watchdog_listen_fd);
		fl_log(2, "[Login_Watchdog]: login watchdog server listen error, errno:%d\n", errno);
		return false;
	}
	...
	fl_log(0,"[Login_Watchdog]: login watchdog server listen on %s : %d\n", addr_tmp, port_tmp);
...
static void * s_watchdog_thread(void * arg)
{
	...
	while (true)
	{
		...
		if (selectn <= 0)
		{
			if (selectn < 0)
			{
				fl_log(2,"[Login_Watchdog]: select error, errno:%d\n", errno);
			}
			continue;
		}
		...
		if (FD_ISSET(s_watchdog_listen_fd, &readset))
		{
			...
			if (-1 == clientfd)
			{
				fl_log(2,"[Login_Watchdog]: Accept client error,errno = %d\n",errno);
			}
			else
			{
				for (i=0;i<32;++i)
